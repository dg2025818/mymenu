...
# --- 페이지 설정 ---
st.set_page_config(
    page_title="고교 급식 칼로리 정밀 비교 (성남고 중심)",
    page_icon="🥗",
    layout="wide"
)
...
st.write("선택한 날짜의 **성남고등학교, 당곡고등학교, 수도여자고등학교** 급식을 칼로리 높은 순으로 정밀 비교합니다.")
...
if not today_meals:
    st.warning(
        ...
        f"- 주말, 공휴일, 재량휴업일, 방학 기간이거나 급식 식단이 등록되지 않은 날짜일 수 있습니다."
    )
else:
    df = pd.DataFrame(today_meals)
    df_sorted = df.sort_values(by="칼로리(kcal)", ascending=False).reset_index(drop=True)

    # 📌 [핵심 추가] 성남고등학교 급식 전용 강조 박스
    sungnam_meal = df_sorted[df_sorted["학교명"] == "성남고등학교"]
    
    st.subheader("🏫 성남고등학교 급식 정보")
    if sungnam_meal.empty:
        st.info("해당 날짜에 성남고등학교 급식 정보가 존재하지 않습니다.")
    else:
        for _, row in sungnam_meal.iterrows():
            sn_rank = df_sorted[df_sorted["학교명"] == "성남고등학교"].index[0] + 1
            st.success(
    ...
    # 1. 종합 요약
    st.subheader(f"📢 {selected_date.strftime('%Y-%m-%d')} 전체 칼로리 순위 및 비교")
